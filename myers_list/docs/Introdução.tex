...
%(Impressiona-me que eu escrevi muito e apenas deu o total de uma página, rir para não chorar)
%(Tmb isto é so a introdução, ent não é preciso ter 3 páginas. De qualquer forma se isso te preocupar 
%podes sempre aumentar a letra para 12 ou 13 pt kkkk)
...
e cdr (aceder à lista excluindo o primeiro elemento) apenas requerem tempo e espaço O(1). No entanto, aceder 
...
Para resolver este problema, Eugene W. Myers \cite{myers1983} propôs uma nova estrutura de dados,
 a Applicative Random-Access Stack, que denominamos Myers List. (Parece que é mais conhecida por Myers Stack). 
 Essa estrutura mantém as operações de cons, car e cdr em tempo constante, permitindo ainda reduzir a complexidade 
 de lookup para uma constante de O(log n), para ser mais preciso 3 log(n) - 5 pointers. Esta melhoria é possível
...
apresentadas no paper, Improved Myers List e Advanced Myers List. Essas estruturas reduziram ainda mais o numero de travessias
 necessárias no pior caso, obtendo uma complexidade de 2 log(n + 1) - 3 e (1 + 1/s) log (n) + s + 9 respetivamente, 
 aproximando-se do limite teórico de log(n + 1) - 1 (teoria da informação).
\\
...
apresentada no paper? \textbf{(ii)} removendo novamente a mesma restrição, é possível aplicar a ideias centrais propostas para a 
Advanced Myers List à Improved Myers List, de modo a melhorar o seu desempenho de lookup e mantendo o cons constante?
\\

Neste relatório, propomos duas novas estruturas de dados e as suas respetivas implementações e bibliotecas, que respondem a estas
