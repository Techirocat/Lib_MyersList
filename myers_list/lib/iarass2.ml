...
let init' v : 'a cell = 
  let rec c : 'a cell = {next = c; jump = c; length = 0; value = v; heigth = 0; shortcut = Skip c} in c 
        ...
        if c_height mod sigma = 0 then 
          if c_height <> 0 then 
            let new_uncle =  l.jump in 
              {next = h; jump = c_jump; length = h_len + 1; value = v; heigth = c_height; shortcut = Skip new_uncle}
          else 
            let new_uncle = c_jump.jump.jump in
              {next = h; jump = c_jump; length = h_len + 1; value = v; heigth = c_height; shortcut = Skip new_uncle}
        else 
          {next = h; jump= c_jump; length = h_len + 1; value = v; heigth = c_height; shortcut = Normal}
      in
      let new_last = 
...
let rec lookup_cost c l cost =   
  ...
  if len = l then 
    ...
    | Skip s when s.length >= l -> lookup_cost s l (cost+1)
    | _ -> 
        if c.jump.length >= l then lookup_cost c.jump l (cost+1)
        else lookup_cost c.next l (cost+1)
  else
      match c.shortcut with 
      | Skip s when len - (pow2 (hei+1)) + 2 >= l -> lookup_cost s l (cost+1)
      | _ -> 
          if len - (pow2 hei)  >= l then lookup_cost c.jump l (cost+1)
          else lookup_cost c.next l (cost+1)
...
let rec lookup c l =   
  ...
  if len = l then 
    ...
    match c.shortcut with 
    | Skip s when len - (pow2 (hei+1)) + 2 >= l -> lookup s l
    | _ -> 
        if len - (pow2 hei)  >= l then lookup c.jump l
        else lookup c.next l
