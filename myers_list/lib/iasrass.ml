      ...
      let c = 
        if c_height mod sigma = 0 then 
          if c_height <> 0 then 
            ...
            let new_uncle = get_jump (get_jump c_jump) in
            Skip {next = h; jump = c_jump; length = h_len + 1; value = v; heigth = c_height; shortcut = new_uncle}
        else 
...
let pick c1 c2 l = 
  if get_length c2 >= l && get_length c2 < get_length c1 then 
    c2 
  else 
    c1   
[@@inline]
...
let rec lookup_cost c l cost =   
  if (get_length c) = l then 
    ...
    match c with 
    | Normal {next = n; jump = j; _ } -> lookup_cost (pick n j l) l (cost + 1)  
    | Skip {next = n; jump = j; shortcut = s; _} -> 
        lookup_cost (pick (pick n j l) s l) l (cost + 1)
